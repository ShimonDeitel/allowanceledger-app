...
struct ContentView: View {
    ...
    @State private var editingItem: LedgerEntry?
    ...
    var body: some View {
        NavigationStack {
                    ...
                    ForEach(store.items) { item in
                        Button {
                            ...
                            VStack(alignment: .leading, spacing: 4) {
                                ...
                                Text(String(describing: item.label))
                                    .font(Theme.bodyFont)
            ...
            .navigationTitle("Allowance Ledger")
            .toolbar {
...
struct EntryFormView: View {
    ...
    let mode: FormMode
    @State private var draft: LedgerEntry

    init(mode: FormMode) {
        self.mode = mode
        switch mode {
        case .add:
            _draft = State(initialValue: LedgerEntry())
        case .edit(let item):
    ...
    var body: some View {
        NavigationStack {
            Form {
                ...
                Toggle("IsCredit", isOn: $draft.isCredit)
                    .accessibilityIdentifier("field_isCredit")
                }
            }
            .onTapGesture {
                hideKeyboard()
