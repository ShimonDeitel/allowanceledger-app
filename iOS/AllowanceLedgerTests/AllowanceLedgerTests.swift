import XCTest
@testable import AllowanceLedger

@MainActor
final class AllowanceLedgerTests: XCTestCase {
    ...
    func testAddIncreasesCount() {
        ...
        let added = store.add(LedgerEntry())
        XCTAssertTrue(added)
    ...
    func testFreeLimitBlocksAddWhenReached() {
        ...
        for _ in 0..<(Store.freeLimit + 5) {
            _ = store.add(LedgerEntry())
        }
        let result = store.add(LedgerEntry())
